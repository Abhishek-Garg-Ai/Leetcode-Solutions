class Solution {
    public void gameOfLife(int[][] board) {
        for(int i=0;i<board.length;i+=1){
            for(int j=0;j<board[0].length;j+=1){
                ...
                if(board[i][j]==1 && count!=3 && count!=4) board[i][j]=2;
    ...
    public int live_neighbors(int[][] board, int cr, int cc){
        int ans=0;
        for(int i=Math.max(0,cr-1);i<=Math.min(board.length-1,cr+1);i+=1)
            for(int j=Math.max(0,cc-1);j<=Math.min(board[0].length-1,cc+1);j+=1)
